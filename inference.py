        ...
        for idx, (tree_id, tree_path) in enumerate(zip(tree_img_list, tree_path_list)):
            ...
            for _ in range(args.iterations):
                for img_seq, img_name_seq, mask_seq, mask_name_seq in tqdm(
                    ...
                    df.loc[count, "Mean Dice/F1"] = (
                        (2 * (meaniou.result().numpy()))
                        / (meaniou.result().numpy() + 1)
                    ) * 100
            ...
            if args.save_img:
                ...
                for name in pred_dict.keys():
                    ...
                    if result.shape[0] > 10:
                        ...
                        cv2.imwrite(
                            os.path.join(
                                args.output, args.species, tree_id, "preds", name
                            ),
                            (preds * 255).astype(np.uint8),
                        )
                        cv2.imwrite(
                            os.path.join(
                                args.output, args.species, tree_id, "variance", name
                            ),
                            (variance * 255).astype(np.uint8),
                        )
                        cv2.imwrite(
                            os.path.join(
                                args.output, args.species, tree_id, "entropy", name
                            ),
                            (entropy * 255).astype(np.uint8),
                        )
                print(" * images saved")
