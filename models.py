...
def ConvLSTM(seq_size, img_height, img_width, droprate):
    ...
    model = Sequential([
    layers.TimeDistributed(ConvLSTMencode(droprate), input_shape=(seq_size, img_height, img_width, 1)),
...
def SegNetSeq(seq_size, img_height, img_width): 
    model = Sequential([
        layers.TimeDistributed(SegNetencode(), input_shape=(seq_size, img_height, img_width, 1)), 
...
def CBAMmodel(input_shape):
    ...
    outputs = layers.experimental.preprocessing.Rescaling(1./255, input_shape=(img_height, img_width, 1))(inputs)
    ...
    #projection
    outputs = layers.Conv2DTranspose(3, 1, padding='same', activation='sigmoid')(outputs)
...
class CBAM(tf.keras.Model):
    def __init__(self, input_shape, chans=1):
        ...
        self.rescale = layers.experimental.preprocessing.Rescaling(1./255, input_shape=(input_shape[0],input_shape[1], 1))
        ...
        self.layproj = layers.Conv2DTranspose(3, 1, padding='same', activation='sigmoid')
...
if __name__ == "__main__": 
    ...
    model_2 = SimpleConvLSTM3(40, 192, 192, 0.2) 
