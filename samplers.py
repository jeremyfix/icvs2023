...
class SimpleDataset:
    ...
    def getDataset(self):
        ...
        return tf.data.Dataset.from_generator(generator,
                              ...
                              output_shapes = (tf.TensorShape(self._info['input_shape']+[1]),tf.TensorShape(self._info['output_shape']+[1])))

    def _generator(self):
        img_list = self._img_list
        lbl_list = self._lbl_list
        for i in range(self.num_samples):
            img = np.expand_dims(cv2.resize(cv2.imread(img_list[i],0),dsize=tuple(self._info['input_shape'])),-1)
            lbl = np.expand_dims(cv2.resize(cv2.imread(lbl_list[i],0),dsize=tuple(self._info['input_shape'])),-1)
            yield (img, lbl)
...
class SequenceDataset(SimpleDataset):
    ...
    def _generate_sequences(self):
        ...
        for img_path, lbl_path in zip(self._img_list, self._lbl_list):
            ...
            image_names = [name for name in sorted(os.listdir(img_path)) if name.split('.')[-1] == 'png']
            label_names = [name for name in sorted(os.listdir(lbl_path)) if name.split('.')[-1] == 'png']
            ...
            pot_lbl_nums = [label_nums[i:i+self.seq_size] for i in range(len(label_nums)-self.seq_size)]
            pot_lbl_names = [label_names[i:i+self.seq_size] for i in range(len(label_names)-self.seq_size)]
            pot_lbl_pathes = [label_pathes[i:i+self.seq_size] for i in range(len(label_pathes)-self.seq_size)]
            pot_img_names = [image_names[i:i+self.seq_size] for i in range(len(image_names)-self.seq_size)]
            pot_img_pathes = [image_pathes[i:i+self.seq_size] for i in range(len(image_pathes)-self.seq_size)]

            # Ensure label sequence continuity
            valid_seq = [((seq[-1] - seq[0]) == (self.seq_size-1)) for seq in pot_lbl_nums ]
            out = [[pot_lbl_names[i], pot_lbl_pathes[i]] for i, cond in enumerate(valid_seq) if cond]
        ...
        for sequence_pair in self._matched_seq:
            for img_path, lbl_path in zip(*sequence_pair):
                if img_path not in self.img_ram.keys():
                	self.img_ram[img_path] = Equalize(cv2.resize(cv2.imread(img_path,-1), tuple(self._info['input_shape'])))
                if lbl_path not in self.lbl_ram.keys():
                	self.lbl_ram[lbl_path] = cv2.resize(cv2.imread(lbl_path,-1), tuple(self._info['input_shape']))
    ...
    def _augment_sequence_pair(self, sequence_pair):
        ...
        for img_path, lbl_path in zip(*sequence_pair):
            # Load and resize
            img = self.img_ram[img_path].copy()#(cv2.imread(img_path,-1), tuple(self._info['input_shape']))
            lbl = self.lbl_ram[lbl_path].copy()#.resize(cv2.imread(lbl_path,-1), tuple(self._info['output_shape']))
            # Apply rotation
            filler = int(img[0,0])
            img = cv2.warpAffine(img, self.rot_mat_img[rot_idx], tuple(self._info['input_shape']), borderValue=filler)
            filler = int(lbl[0,0])
            lbl = cv2.warpAffine(lbl, self.rot_mat_lbl[rot_idx], tuple(self._info['output_shape']), borderValue=filler)
            # Apply flip
    ...
    def _ready_augmentation(self):
        ...
        for angle in np.linspace(0,360,8)[:-1]:
            self.rot_mat_img.append(cv2.getRotationMatrix2D((self._info['input_shape'][0]/2, self._info['input_shape'][1]/2), angle, 1))
            self.rot_mat_lbl.append(cv2.getRotationMatrix2D((self._info['output_shape'][0]/2, self._info['output_shape'][1]/2), angle, 1))
    ...
    def getDataset(self):
        ...
        return tf.data.Dataset.from_generator(generator,
                              args=[],
                              output_types=(tf.float32, tf.uint8),
                              output_shapes = (tf.TensorShape([self._info['seq_size']]+self._info['input_shape']+[1]),[self._info['seq_size']]+tf.TensorShape(self._info['output_shape']+[1])))
