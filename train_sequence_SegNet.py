...
#weights = "/home/GPU/skhazem/Knots_segmentation/main/KnotsFromContours_SegNet/models/20230109-190741/new_contour_model_112/epoch_112.ckpt" 
weights = "./KnotsFromContours_SegNet_2/models/20230118-135815/new_contour_model_2/epoch_2.ckpt"
def parse():
...
model = SegNetSeq(TRN_SIS._info['seq_size'], TRN_SIS._info['input_shape'][0], TRN_SIS._info['input_shape'][1])
if args.weights:
...
def train_step(model, optimizer, x_train, y_train):
    ...
    train_meaniou.update_state(tf.cast(y_train[:,:,:,:,0],tf.int32), tf.cast(tf.math.round(pred_copy[:,:,:,:,0]),tf.int32))
...
def val_step(model, x_val, y_val):
    ...
    pred_copy = np.copy(predictions)
    val_meaniou.update_state(tf.cast(y_val[:,:,:,:,0],tf.int32), tf.cast(tf.math.round(pred_copy[:,:,:,:,0]),tf.int32))
...
for epoch in range(EPOCHS):
    for step, (x_train, y_train) in enumerate(train_ds.batch(batch_size)):
        ...
        if step%max_steps_prct == 0: 
            ...
            print (template.format(epoch+1,
                                   ...
                                   ((2*(train_meaniou.result().numpy())) / (train_meaniou.result().numpy() + 1))*100))
            with train_summary_writer.as_default():
                video_summary('prediction',model(x_train).numpy()[:3], step = epoch*max_steps+step)
                video_summary('ground_truth',y_train.numpy()[:3], step = epoch*max_steps+step)
                overlay = tf.dtypes.cast((x_train[:3] - np.min(x_train[:3]))/(np.max(x_train[:3]) - np.min(x_train[:3]))*255, tf.uint8).numpy()
                overlay = np.concatenate((overlay,)*3, axis=-1) #convert to rgb
                overlay[:,:,:,:,1] = y_train.numpy()[:3,:,:,:,0]
                video_summary('groundtruth_with_overlay',overlay, step = epoch*max_steps+step)
                overlay = tf.dtypes.cast((x_train[:3] - np.min(x_train[:3]))/(np.max(x_train[:3]) - np.min(x_train[:3]))*255, tf.uint8).numpy()
                overlay = np.concatenate((overlay,)*3, axis=-1) #convert to rgb
                overlay[:,:,:,:,1] = np.max(tf.dtypes.cast(255*model(x_train[:3]), tf.uint8).numpy(), axis=-1)
                video_summary('prediction_with_overlay',overlay, step = epoch*max_steps+step)
                tf.summary.scalar('loss', train_loss.result(), step = epoch*max_steps+step)
                tf.summary.scalar('miou', train_meaniou.result(), step = epoch*max_steps+step)
                tf.summary.scalar('accuracy', train_accuracy.result(), step = epoch*max_steps+step)
                tf.summary.flush()
    ...
    with val_summary_writer.as_default():
        ...
        tf.summary.scalar('accuracy', val_accuracy.result(), step=epoch*max_steps+step)
        tf.summary.scalar('miou', val_meaniou.result(), step=epoch*max_steps+step)
        video_summary('test_prediction',model(x_val).numpy(), step = epoch*max_steps+step)
        ...
        video_summary('test_groundtruth_with_overlay',overlay, step = epoch*max_steps+step)
        overlay = tf.dtypes.cast(x_val/256, tf.uint8).numpy()
        overlay = np.concatenate((overlay,)*3, axis=-1) #convert to rgb
        overlay[:,:,:,:,1] = np.max(tf.dtypes.cast(255*model(x_val), tf.uint8).numpy(), axis=-1)
        #print(np.max(overlay),np.max(np.max(tf.dtypes.cast(255*model(x_train), tf.uint8).numpy(), axis=-1)))
        video_summary('test_prediction_with_overlay',overlay, step = epoch*max_steps+step)
      
    template = 'Epoch {}, Loss: {}, Pixel-Accuracy: {}, Test Loss: {}, Test Pixel-Accuracy: {}, Train MeanIoU: {}, Val MeanIoU:{}, Val MeanF1: {}'
    print (template.format(epoch+1,train_loss.result(), train_accuracy.result()*100,val_loss.result(), val_accuracy.result()*100, train_meaniou.result()*100, val_meaniou.result()*100, ((2*(val_meaniou.result().numpy())) / (val_meaniou.result().numpy() + 1))*100)) 
    ...
    train_meaniou.reset_states()
    model.save(os.path.join(args.output_path,'models',current_time,'new_contour_model_'+str(epoch)))
    checkpoint_path = os.path.join(args.output_path,'models',current_time,'new_contour_model_'+str(epoch)+"/epoch_"+str(epoch)+".ckpt")
